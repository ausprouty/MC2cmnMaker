...
export default {
	...
	async addNote(noteid, route, noteText){
		var source = localStorage.getItem('mc2NoteSourceCmn');
		if (source == 'database'){
	...
	async notesFromDatabase(route){
			try {
				...
				await sqlite.closeConnection("db_notes_cmn");
				return res.values
			} catch (err) {
				console.log (' error in SQLite Service notes from database for ' + route)
				console.log(`Error: ${err}`);
				localStorage.removeItem('mc2NoteSourceCmn')
	...
	async openDatabase(){
		try {
			...
			const isConn = (await sqlite.isConnection("db_notes_cmn")).result;
			//console.log(`after isConnection ${isConn}`);
			let db;
			if (ret.result && isConn) {
				//console.log("I am retreiving connection")
				db = await sqlite.retrieveConnection("db_notes_cmn");
			} else {
				//console.log("I am creating  connection")
				db = await sqlite.createConnection("db_notes_cmn", false, "no-encryption", 1);
			}
			await db.open();
			return db
		} catch (err) {
  ...
  // Dealing with Textarea Height
	...
	async getDataSource(){
		var noteSource = localStorage.getItem('mc2NoteSourceCmn')
		if (!noteSource){
	...
	async createDataStore(){
		...
		try {
			...
			if (res.changes && res.changes.changes && res.changes.changes < 0) {
				...
				localStorage.setItem('mc2NoteSourceCmn', 'localstorage')
				return;
			}
			await sqlite.closeConnection("db_notes_cmn");
			localStorage.setItem('mc2NoteSourceCmn', 'database')
		} catch (err) {
			localStorage.setItem('mc2NoteSourceCmn', 'localstorage')
		}
		return localStorage.getItem('mc2NoteSourceCmn')
	}
}
